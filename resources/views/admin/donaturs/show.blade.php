            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                <div class="flex flex-row gap-x-16">
                    ...
                    <div>
                        ...
                        <h3 class="text-indigo-950 text-xl font-bold">{{ $donatur->fundraising->name }}</h3>
                        <p class="text-slate-500 text-sm">Target Rp
                            {{ number_format($donatur->fundraising->target_amount, 0, ',', '.') }}</p>
                    </div>
                </div>
                <hr class="my-5">
                <h3 class="text-indigo-950 text-xl font-bold mb-5">Bukti Transfer</h3>
                <img src="{{ Storage::url($donatur->proof) }}" alt="{{ Storage::url($donatur->proof) }}"
                    class="rounded-2xl object-cover w-[300px] h-[200px] mb-3">
